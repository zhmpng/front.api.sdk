    ...
    internal sealed class ButtonsTester : IDisposable
    {
        ...
        public ButtonsTester()
        {
            subscriptions = new CompositeDisposable
            {
                ...
                Operations.AddButtonToPaymentScreen("Sample Plugin", x =>
                {
                    x.vm.ShowOkPopup("Sample plugin", "Payment screen click on sample button");
                }, IikoIcon),
                Operations.AddToggleButtonToPaymentScreen(x =>
                {
                    return new ToggleButtonState { Caption = "Sample Checked", IsChecked = true, Icon = IikoIcon};
                }, 
                x =>
                {
                    var newState = !x.state.IsChecked;
                    var newCaption = newState ? "Sample Checked" : "Sample Unchecked";
                    x.vm.ShowOkPopup("Sample plugin", $"Payment screen click on toggle button. New state: {newState}");
                    return new ToggleButtonState { Caption = newCaption, IsChecked = newState, Icon = x.state.Icon };
                })
            };
        }
